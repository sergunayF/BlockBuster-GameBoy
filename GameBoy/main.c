...
void _bombDraw(INT8 x, INT8 y, INT8 anim) {

	set_sprite_tile(22, 30 + anim);
	move_sprite(22, 8 + grid + (x * 16), 16 + (grid * 4) + y * 16);
	set_sprite_tile(23, 32 + anim);
	move_sprite(23, 8 + grid + (x * 16) + 8, 16 + (grid * 4) + y * 16);
}
...
void _boom(void) {
	...
	for (INT8 j = -1; j < 2; j++) {
		for (INT8 i = -1; i < 2; i++) {
			...
			switch (field[_bomb.x + j][_bomb.y + i]) {
			case 1: _countStone += 1; field[_bomb.x + j][_bomb.y + i] = 0; break;
			case 2: _countBrick += 1; field[_bomb.x + j][_bomb.y + i] = 0; break;
			case 3: _countBrick += 1; field[_bomb.x + j][_bomb.y + i] = 0; break;
			case 4: _countBrick += 1; field[_bomb.x + j][_bomb.y + i] = 0; break;
			case 5: _countCoin += 1; field[_bomb.x + j][_bomb.y + i] = 0; break;
			case 6: _countWater += 1; field[_bomb.x + j][_bomb.y + i] = 0; break;
			//case 2: _countBrick += 1; field[_bomb.x - j - 1][_bomb.y - i - 1] = 0; continue;
			}
		}
	}

	field[_bomb.x][_bomb.y] = 0; _bomb.anim = 0; _spawn(7);
...
void _draw(void) {
	...
	for (INT8 i = 0; i < fieldY; i++) {
		for (INT8 j = 0; j < fieldX; j++) {
			...
			if (field[j][i] == 7) { _bomb.x = j; _bomb.y = i; _bombDraw(j, i, _bomb.anim * 4); }
...
int main(void) {
	...
	while (_countBomb > 0) { _spawn(7); _countBomb -= 1; }
	...
	while (1) {
		...
		if (_bomb.timer == 30 && _bomb.anim == 3) { _bomb.timer = 0; _boom(); }
		if (_bomb.timer == 30 && _bomb.anim < 3) { _bomb.anim++; _bomb.timer = 0; }
